...
window.addEventListener('load', (_) => __awaiter(void 0, void 0, void 0, function* () {
    // const newMemory = new WebAssembly.Memory({
    //     initial: 10,
    //     maximum: 100,
    //     shared: true,
    //   });
    const wa = yield WebAssembly.instantiateStreaming(fetch('./js/main.wasm'), {
        "env": make_environment({
            malloc,
            free,
            RunTimeException,
            "print": (strPtr) => { console.log(DecodeStringFromMemory(strPtr)); },
        }),
    });
    wasm_exports = wa.instance.exports;
    heap_base = wasm_exports.__heap_base.value;
    const main = wasm_exports.main;
    console.log(main());
